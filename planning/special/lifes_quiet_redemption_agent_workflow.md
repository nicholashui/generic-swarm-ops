# Special skill adoption plan — `lifes_quiet_redemption_agent_workflow`

| Field | Value |
|-------|-------|
| **Skill id** | `lifes_quiet_redemption_agent_workflow` |
| **Source** | `C:/Project/va-agent-swarm/study/lifes_quiet_redemption_agent_workflow.md` |
| **In-pack corpus mirror** | `business/video/corpus/study/lifes_quiet_redemption_agent_workflow.md` (if present under video corpus) |
| **Source size** | 356 lines / 47,105 chars / ~184 table rows detected |
...
Worked example for archetype E / LQR family; deepen DNA from LQR SVGs.
...
- Workflow/theme: 0. Visual Diagrams (read this first)
- Workflow/theme: 1. Pipeline Overview — Phase → Owning Agents → Service
- Workflow/theme: 2. Scene-by-Scene Production Matrix
- Workflow/theme: 3. Agent Service Catalogue (What Each Agent Actually Does on This Film)
- Workflow/theme: 4. Cross-Cutting Services Applied Throughout
- Workflow/theme: 5. Quality Gate Ladder (Per Shot & Per Phase)
- Workflow/theme: 6. Delivery Variants & Outlet Specs
- Workflow/theme: 7. Recommended Tool / Model Stack (June 2026)
- Workflow/theme: 8. Series & Scalability (Reusing the Swarm)
- Workflow/theme: 9. Research Sources → Findings → Workflow Implication
...
4. [ ] Read source: `C:/Project/va-agent-swarm/study/lifes_quiet_redemption_agent_workflow.md` and mirror under corpus if present.
5. [ ] Map FRs → existing agents/DNA (avoid duplicate agents).
6. [ ] Create feature branch `special/lifes_quiet_redemption_agent_workflow`.
...
#### Sprint 1 — 0. Visual Diagrams (read this first)
- **Goal:** Implement/test the smallest slice that proves progress on «0. Visual Diagrams (read this first)» from the source outline.
- **Work:**
...
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/lifes_quiet_redemption_agent_workflow_sprint1_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
#### Sprint 2 — 1. Pipeline Overview — Phase → Owning Agents → Service
- **Goal:** Implement/test the smallest slice that proves progress on «1. Pipeline Overview — Phase → Owning Agents → Service» from the source outline.
- **Work:**
  - Map theme to pack artifacts (`agent_spec.json`, SPEC.md, DNA step, or host service).
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/lifes_quiet_redemption_agent_workflow_sprint2_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
#### Sprint 3 — 2. Scene-by-Scene Production Matrix
- **Goal:** Implement/test the smallest slice that proves progress on «2. Scene-by-Scene Production Matrix» from the source outline.
- **Work:**
  - Map theme to pack artifacts (`agent_spec.json`, SPEC.md, DNA step, or host service).
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/lifes_quiet_redemption_agent_workflow_sprint3_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
#### Sprint 4 — 3. Agent Service Catalogue (What Each Agent Actually Does on This Film)
- **Goal:** Implement/test the smallest slice that proves progress on «3. Agent Service Catalogue (What Each Agent Actually Does on This Film)» from the source outline.
- **Work:**
  - Map theme to pack artifacts (`agent_spec.json`, SPEC.md, DNA step, or host service).
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/lifes_quiet_redemption_agent_workflow_sprint4_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
#### Sprint 5 — 4. Cross-Cutting Services Applied Throughout
- **Goal:** Implement/test the smallest slice that proves progress on «4. Cross-Cutting Services Applied Throughout» from the source outline.
- **Work:**
  - Map theme to pack artifacts (`agent_spec.json`, SPEC.md, DNA step, or host service).
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/lifes_quiet_redemption_agent_workflow_sprint5_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
...
Suggested path: `backend/app/tests/unit/test_special_lifes_quiet_redemption_agent_workflow.py`
...
This source: **356 lines** → plan capacity accordingly.
...
| Level | Heading |
|------:|---------|
| 1 | "Life's Quiet Redemption" — Agent-Orchestrated Production Workflow |
| 2 | 0. Visual Diagrams (read this first) |
| 3 | D1 · Pipeline Overview |
| 3 | D2 · Scene Flow, Emotional Arc & Retention Bands |
| 3 | D3 · Per-Shot Generation Loop |
| 3 | D4 · Character-Consistency Identity Stack |
| 3 | D5 · Engine Routing (RouterAgent Tiers) |
| 3 | D6 · Quality Gate Ladder & VBench Scorecard |
| 2 | 1. Pipeline Overview — Phase → Owning Agents → Service |
| 2 | 2. Scene-by-Scene Production Matrix |
| 2 | 3. Agent Service Catalogue (What Each Agent Actually Does on This Film) |
| 3 | 3.1 Orchestration & Planning |
| 3 | 3.2 Above-the-Line & Story |
| 3 | 3.3 Look, Character & Continuity |
| 3 | 3.4 Generation, Camera & Audio |
| 3 | 3.5 Post, QA, Compliance & Delivery |
| 2 | 4. Cross-Cutting Services Applied Throughout |
| 2 | 5. Quality Gate Ladder (Per Shot & Per Phase) |
| 2 | 6. Delivery Variants & Outlet Specs |
| 2 | 7. Recommended Tool / Model Stack (June 2026) |
| 2 | 8. Series & Scalability (Reusing the Swarm) |
| 1 | PART B — Research-Informed Quality Upgrades (June 2026) |
| 2 | 9. Research Sources → Findings → Workflow Implication |
| 2 | 10. YouTube Marketing Upgrades (mapped to agents) |
| 2 | 11. Generation Engine Upgrade — Add Grok Imagine to the Stack |
| 2 | 12. Character-Consistency Upgrade (the film's hardest problem) |
| 2 | 13. Evaluation Upgrade — VBench-Grade QC Scorecard |
| 2 | 14. Orchestration Upgrade — 3E Loop + MCTS Search |
| 2 | 15. Revised Opening & Shorts Cards (concrete deltas) |
| 3 | Sources |
...
| Artifact | Location |
|----------|----------|
| This plan | `planning/special/lifes_quiet_redemption_agent_workflow.md` |
| Upstream study | `C:/Project/va-agent-swarm/study/lifes_quiet_redemption_agent_workflow.md` |
...
*End of plan — `lifes_quiet_redemption_agent_workflow` — generated 2026-07-13*
