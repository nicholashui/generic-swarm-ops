# Special skill adoption plan — `aesthetics_agent`

| Field | Value |
|-------|-------|
| **Skill id** | `aesthetics_agent` |
| **Source** | `C:/Project/va-agent-swarm/study/aesthetics_agent_functional_specification.md` |
| **In-pack corpus mirror** | `business/video/corpus/study/aesthetics_agent_functional_specification.md` (if present under video corpus) |
| **Source size** | 384 lines / 29,828 chars / ~43 table rows detected |
...
Maps to visual look/style consistency in video Domain Pack; do not fork control plane.
...
- Aesthetics must not be hackable.**
...
- Input (JSON).**
- Output — `AestheticVerdict` (JSON + Markdown).**
- `screen` — fast scalar gate for high-volume candidate culling (regression head only).
- Statefulness.** Per-project memory of accepted/rejected artifacts ratchets the profile and feeds Reflexion-style episodic memory (consistent with swarm principle "Continuous Self-Improvement").
- Non-functional.** Fast-tier screening ≤ 800 ms/artifact; deep-tier ≤ 8 s; horizontal GPU autoscale; deterministic given fixed profile + model versions (reproducibility for audits).
...
- Critique Bus.** Verdicts are published on the swarm's structured critique bus (`SYSTEM_REFERENCE.md` §7.1) with `critique_type: "aesthetic_feedback"`, `severity`, `rubric_score`, and `artifact_ref`, so any agent can react asynchronously.
...
- Critic Loop (evaluate).** `artifact → ensemble score → decompose → gate by intent/emotion → anti-hack check → AestheticVerdict`.
...
4. [ ] Read source: `C:/Project/va-agent-swarm/study/aesthetics_agent_functional_specification.md` and mirror under corpus if present.
5. [ ] Map FRs → existing agents/DNA (avoid duplicate agents).
6. [ ] Create feature branch `special/aesthetics_agent`.
...
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/aesthetics_agent_sprint1_notes.md` (optional) or this plan's risk log.
...
- **Work:**
  ...
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/aesthetics_agent_sprint2_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
...
- **Work:**
  ...
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/aesthetics_agent_sprint3_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
...
- **Work:**
  ...
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/aesthetics_agent_sprint4_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
...
- **Work:**
  ...
  - Implement behind feature flag / sandbox; no production_ready until gates pass.
  - Add/extend unit tests for the slice.
- **Spec checkpoint:** Diff behavior vs source headings related to this theme; log gaps in `planning/special/aesthetics_agent_sprint5_notes.md` (optional) or this plan's risk log.
- **Exit:** Automated tests green + checklist of covered source requirements updated.
...
Suggested path: `backend/app/tests/unit/test_special_aesthetics_agent.py`
...
This source: **384 lines** → plan capacity accordingly.
...
| 3 | 2. Background: From "Artiste Sense" to Computational Aesthetics |
| 3 | 3. The Deep Rethink — Five Reframings |
| 3 | 4. Formal Aesthetic Model |
| 3 | 5. Architecture |
| 3 | 6. Aesthetic Dimensions (the Decomposed Rubric) |
| 3 | 7. Functional Requirements |
| 3 | 8. Integration with the VA-Agent-Swarm |
| 3 | 9. The Three Operating Loops |
| 3 | 10. Personalization: the `AestheticProfile` |
| 3 | 11. Reward Hacking, Failure Modes & Defenses |
| 3 | 12. Technical Architecture & Implementation Guidelines |
| 3 | 13. Evaluation & Success Criteria |
| 3 | 14. Limitations & Future Directions |
| 3 | 15. References (Curated, 2024–2026) |
...
| Artifact | Location |
|----------|----------|
| This plan | `planning/special/aesthetics_agent.md` |
| Upstream study | `C:/Project/va-agent-swarm/study/aesthetics_agent_functional_specification.md` |
...
*End of plan — `aesthetics_agent` — generated 2026-07-13*
