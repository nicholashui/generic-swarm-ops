...
**Purpose:** Build an executable starter repository that downloads, audits, curates, and synchronizes reference sources into a **Trae IDE** workspace, including project rules, custom agents, skills, commands, MCP configuration, and supporting docs.  
**Current spec version:** `1.2.0-trae-only`  
**Last updated:** 2026-07-07  

---

## How to Use This Boilerplate With Trae IDE

This repository is a boilerplate specification.

It targets **Trae IDE** and **Grok Build** as dual agent harnesses (`.trae/` and `.grok/` via `npm run sync`). Other editor-specific outputs such as `CLAUDE.md`, `GEMINI.md`, `.cursor/`, `.codex/`, `.opencode/`, or GitHub Copilot instruction files remain out of scope for this variant.

A human may ask a coding agent to create a new project from this file by using a prompt like:

```text
Create a new project named <PROJECT_NAME> based on <PATH_TO_STARTER_MD>.

Use starter.md as the source-of-truth implementation contract.
...
You must:
1. Read starter.md fully.
2. Create the new project directory.
3. Create all directories and files required by starter.md.
4. Replace boilerplate metadata with the new project name and purpose.
5. Copy starter.md into the new project.
...
```text
Implement starter.md in the current repository.

Use starter.md as the complete implementation contract.

Create all required files, directories, manifests, scripts, docs, tests, and Trae workspace files.
...
|---|---|---|
| `PROJECT_NAME` | Name of the generated project | `starter_generated` |
| `PROJECT_PURPOSE` | Short description of the generated project | `Trae IDE starter repository` |
...
| `AGENT_SUPPORT` | Supported IDE / agent target | `trae` |
...
When a coding agent receives this file and is asked to implement it, the agent **MUST** create a real executable project for Trae IDE.

The agent **MUST NOT** stop after writing Markdown.

The agent **MUST**:

1. Create the `starter/` repository structure.
...
17. Generate Trae workspace outputs under `.trae/` and supporting docs.
18. Run the bootstrap validation commands.
19. Report success or exact blockers.
...
{
  "name": "starter",
  "version": "1.2.0-trae-only",
  "private": true,
  "type": "module",
  "description": "Executable starter repo for downloading, auditing, curating, and syncing Trae IDE workspace sources.",
  "scripts": {
...
## 4. Mandatory Upstream Source Download Manifest

The coding agent **MUST** create this exact file.

The upstream list may include sources from non-Trae ecosystems, but in this Trae-only variant they are treated strictly as **reference inputs** for generating curated Trae-compatible rules, skills, commands, MCP configs, and agent docs.
...
```json
{
  ...
  "docs": [
    {
      "id": "trae-agent-docs",
      "name": "Trae Create and Manage Agents",
      "url": "https://docs.trae.ai/ide/agent?_lang=en",
      "target": "external/docs/trae-agents.md",
      "enabled": true,
      "purpose": "Official Trae custom-agent model, built-in tool selection, and MCP attachment reference."
    },
    {
      "id": "trae-skills-docs",
      "name": "Trae Skills Docs",
      "url": "https://docs.trae.ai/ide/skills?_lang=en",
      "target": "external/docs/trae-skills.md",
      "enabled": true,
      "purpose": "Official Trae skill model, SKILL.md structure, and project-skill workflow reference."
    },
    {
      "id": "trae-changelog",
      "name": "Trae Changelog",
      "url": "https://www.trae.ai/changelog",
      "target": "external/docs/trae-changelog.md",
      "enabled": true,
      "purpose": "Official change log for Trae features such as command files, rules behavior, hooks, and IDE capabilities."
    },
    {
      ...
      "enabled": true,
      "purpose": "MCP concepts, protocol docs, client/server design, and safety references used by Trae agents."
    }
  ]
}
```
...
5. Copy `starter.md` into the new project root.
...
The coding agent **MUST** implement a first working Trae sync framework, even if the first adapter pass is minimal.
...
This file should summarize supported Trae agents/workflows, shared rules, validation commands, and the policy that downloaded sources are reference material until audited.
