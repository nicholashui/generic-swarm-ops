...
export function getSourceManifest() {
  return JSON.parse(JSON.stringify(SOURCE_MANIFEST));
}

export function getDocsManifest() {
  return JSON.parse(JSON.stringify(DOCS_MANIFEST));
}

export function validateSourceManifest(manifest) {
  if (!manifest || !Array.isArray(manifest.sources)) {
    throw new Error("sources/manifest.json must contain a sources array.");
  }

  const seenIds = new Set();
  for (const source of manifest.sources) {
    for (const field of ["id", "url", "target", "priority", "tier", "import_policy"]) {
      if (!source[field]) {
        throw new Error(`Source ${source.id || "<unknown>"} is missing required field ${field}.`);
      }
    }
    ...
    if (!source.target.startsWith("external/sources/")) {
      throw new Error(`Source ${source.id} target must start with external/sources/.`);
    }
  }
  return true;
}

export function selectSources(manifest, profile = "all") {
  validateSourceManifest(manifest);
  const enabledSources = manifest.sources.filter((source) => source.enabled === true);
  if (profile === "all") {
    return enabledSources;
  }
  return enabledSources.filter((source) => source.tier === profile);
}

export function toAbsoluteTarget(rootDir, relativeTarget) {
  return path.join(rootDir, relativeTarget);
}
