# User guide: Approvals queue

**Screen:** `/app/approvals`  
**Who:** Operators with `approvals:read` (or stronger)
...
1. Open Approvals when a run is waiting.
2. Read risk, workflow context, and proposed action carefully.
3. Approve only when policy and evidence support the action; otherwise reject with a clear reason.
4. Confirm the workflow run continues or fails closed after your decision.
...
- Spec: `/docs/app/approvals/spec.md`
