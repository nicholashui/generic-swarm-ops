...
- **Operator product slice (2026-07-14):** Domains UI — Recommend Workflow + Special Skills panels; API `special-skills` list; client methods on `backendApi`; tests `test_video_special_skills_api` + FE `product-slice-video-ops.test.ts`.
- **Executable product (2026-07-13):** `EXECUTABLE_PRODUCT.md` — E1 operator path + viral-hook video DNA e2e proven; health/ready (Postgres); `scripts/business/check_executable_product.py`; tests `test_executable_product_health.py`. Residuals: live media, deep A–J production_ready, Discover FE.
- **Migration COMPLETE record (2026-07-13):** `MIGRATION_COMPLETE.md` — knowledge-standalone + N3 pack DoD closed (325 corpus, 114 inventory, va_only=0, tests green). Residuals: live media, deep DNA production_ready, Discover FE, fleet true-100 agent scores (explicit non-goals of this migration).
- **Special skills SDD plans (2026-07-13):** Executed `generate_special_skills.md` → 17 plans in `planning/special/*.md` (+ index README) from va `study/` list; regenerator `scripts/business/generate_special_skills_plans.py`.
- **Special skills host MVP + scores (2026-07-13):** Integrations under `business/video/special_skills/<id>/` (integration.json + SKILL.md); scores in `planning/special/special_skill_impl_score.md` + root pointer `special_skill_impl_score.md` (mean ~92.9, 0× full mark 100); tests `test_special_skills_inventory.py`; implementer `scripts/business/implement_and_score_special_skills.py`.
- **Video workflow selection v1 (2026-07-13):** `business/video/archetype_registry.json` (A–J + S1–S7); selector `backend/app/domain/workflows/archetype_selector.py`; API `GET/POST .../domains/video/archetypes` + `recommend-workflow`; CLI `scripts/business/recommend_video_workflow.py`. Deepened DNA B (UGC ad) + E (short film) to phased_v1 from va §3. HiTL confirm default before launch.
- **Migration v1 honest (2026-07-13):** Reverted false fleet-100. Role-family research sections applied; per-agent scores in `planning/migration/agent_impl_v1_mark.md` (mean 93.8; true 100 count=0). `tasks.md` status=v1 honest reopened.
