...
**版本：** 2.4  
**日期：** 2026-07-13  
**狀態：** **主機上 N3 實作完成**（improvements 波次 0–5）——策略內容沿用 v2.3 rethink；執行證據已在倉庫內  
**作者：** 依 `review_adoption.md` 審閱兩個本機儲存庫  
**審核來源（深度本機掃描 2026-07-10；實作狀態 2026-07-13）：**
...
- 執行真相源：`improvements.md` v1.0 · SDD：`planning/improvement/wave-{0..5}/` · 閘門：`python scripts/business/inventory_check.py` → `INVENTORY PASS count=114 n3=complete`

**英文原文：** `adoption.md`（本檔為對應繁體中文版，與 **v2.4** 對齊）

### 實作狀態（v2.4——improvements 波次 0–5 之後）

| 波次 | 目標 | 狀態 |
|------|------|------|
| **0** | 目錄、schemas、盤點閘門、register CLI | **完成** |
| **1** | ALC runtime + domain register API + example 包 + FE DomainPackPanel | **完成** |
| **2** | 脊柱 + viral-hook DNA、video 工具 stubs、脊柱 E2E | **完成** |
| **3** | 共演化、lesson utility、LQR evals、governance review | **完成** |
| **4** | 多包負載／安全、第三包、runbook | **完成** |
| **5** | 完整名冊 registered + 流程 DNA／文件 + N3 CI | **完成** |

**N3（誠實，依 rethink 表述）：** 全部 **114** 代理人 ≥ **L1**（registered + standby／router 可達）；脊柱 ≥ **L2**；流程覆蓋為 DNA 或包內連結（**va-only = 0**）；盤點 CI + `.github/workflows/n3-inventory.yml`。

**仍屬 N3 之後（不宣稱完成）：** 每一位專精的深度 L2 啟用；真實 Sora／Veo 等媒體供應商；進階時間軸 FE；Temporal 適配器。薄 DNA stub 是刻意的 N3 表達深度。

證據：`planning/improvement/chain-run-report.md`、`planning/improvement/wave-5/completion-report.md`、`docs/domain-packs.md`。
...
**基線掃描（2026-07-10）**——為稽核保留。標 **[已關閉]** 者為當時缺口，已由波次 0–5（2026-07-12/13）關閉。
...
| **generic** API 路由 | auth、users、orgs、agents、tools、workflows、workflow_runs、approvals、governance、knowledge、memory、evaluations、audit_logs、processes、evolution、improvement、loops、settings、health——**[+]** `domains`（register、video N3 status）**[已關閉 波次 1/5]** |
| **generic** business schemas | WorkflowDNA、EvaluationCard、DecisionRequirementCard、EventLog——**[+]** agent-spec、domain-manifest、learning-log **[已關閉 波次 0]** |
| **generic** 影片包 | **`business/video/` 已存在**——114 代理人、ROSTER/MAP、standby/router、process_coverage、14 DNA、保留政策 **[已關閉 波次 0–5]** |
| **generic** 種子代理人 | 5 個營運代理人：`business_orchestrator`、`quality_compliance_agent`、`execution_agent`、`finance_ops_agent`、`communications_agent`（註冊後另含包代理人） |
| **generic** 工具適配器 | 營運工具 + **`video_*` stubs**（media、script、qc、package）**[已關閉 波次 2]** |
| **generic** 金任務 | 平台 customer-onboarding + **影片包** golden／LQR fixtures **[已關閉 波次 2–3]** |
| **generic** lessons | 工作流層級 lessons + **`agent_id` ALC lessons**／inject／metrics **[已關閉 波次 1]** |
| **generic** 學習缺口（掃描時） | *當時：* 無 `agent_id`——**現已關閉**（ALC）。殘餘：逐步深度 L2／真實媒體。 |
| **generic** 前端領域 | 營運主控台 + **DomainPackPanel**、**LessonUtilityPanel**、**VideoN3RosterPanel** **[已關閉 波次 1/3/5]** |
| **generic** 多包 | `example_research` + `example_education` + 負載／隔離／安全測試 **[已關閉 波次 1/4]** |
...
**平台**（截至 improvements 波次 0–5，2026-07-13）

- [x] 對 `requires_alc: true` 的包強制 ALC  
- [x] 影片脊柱 E2E 綠（編排器向下路徑）  
- [x] 第二領域包可註冊且無需改 runtime 核心（`example_research`；第三包：`example_education`）  
- [x] 永不繞過 `sandbox_only`  
- [x] 單元套件綠（完整 `app/tests/unit`；各波次測試）——每 PR 持續保護 E1  
- [x] 缺口說明已更新於 handoff／`planning/improvement/`  

**完整 va 採用（N3）**（誠實出口：L1 名冊 + 已索引流程 + 脊柱 L2）

- [x] `business/video/agents/` + MAP 數 == 完整 va 名冊（無省略）  
- [x] 10 類齊；元代理人含 orchestrator／planner／router／judge  
- [x] 全部 va 業務流程已索引並以 DNA／包內連結表示（`process_coverage.json`，va_only=0）  
- [x] 每位代理人編排器可達（DNA 步驟或 standby_pool）  
- [x] 保留政策強制（刪除名冊代理人則 CI 失敗；`policies/roster-retention.md`）  
- [x] 啟用波次已文件；代理人以磁碟 **registered** + standby 留在專案；深度 **active** L2 仍可繼續分波  

**仍開放（N3 之後的產品深度——非上述 N3 表述之阻擋項）**

- [ ] 真實媒體供應商（取代 stubs）含預算／速率限制  
- [ ] 全部 114 專精深度 L2 啟用（超出薄 DNA + registered 目錄）  
- [ ] 進階時間軸／管線 FE（P2）
...
文件：`adoption.md`／`adoption_hk.md`（**v2.4**）、`docs/domain-packs.md`（已存在）、`docs/add-domain-pack-runbook.md`、`docs/domain-pack-versioning-matrix.md`、`business/video/README.md`、`planning/improvement/wave-*/` 波次 SDD 與完成報告、structure／backend／frontend；va SYSTEM_REFERENCE 可加「已在 generic pack 路徑實作」橫幅。
...
| **v2.4 實作狀態（波次 0–5 完成）** | **文首「實作狀態」、研究快照 [已關閉]、§7.5 DoD、§13 變更紀錄；對齊英文 adoption.md v2.4** |
...
| 2.3 | 2026-07-10 | **重新思考：** L0／L1／L2 成熟度；目錄 ≠ 完整 SPEC；ALC 硬閘僅 active；先流程索引後 DNA 深度；ops vs `video.orchestrator`；N3 完成 vs 生產級誠實表述 |
| **2.4** | **2026-07-13** | **N3 實作狀態：** improvements 波次 0–5 在主機完成。標題狀態、實作表、研究快照缺口標「已關閉」、§7.5 DoD 勾選。證據：`planning/improvement/`、盤點 `n3=complete`。殘餘：深度 L2／真實媒體／進階 FE。本 HK 與英文 **v2.4** 對齊 |
| **2.4.1** | **2026-07-13** | **影片語料獨立：** 執行 `migration_plan.md`——va 全文 325 檔入 `business/video/corpus/`；114 份 SPEC.md 已展開（不再僅 refer）。 |
