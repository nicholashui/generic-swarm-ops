...
class LessonLibrary:
    def __init__(self, items: list[dict[str, Any]] | None = None, capacity: int = 200):
        ...
        self.lessons: list[Lesson] = [lesson_from_dict(i) for i in (items or []) if i.get("text")]

    def add(self, lesson: Lesson) -> Lesson | None:
        for existing in self.lessons:
            if existing.text == lesson.text and existing.workflow_id == lesson.workflow_id:
                return existing
        self.lessons.append(lesson)
        self._consolidate()
        return lesson

    def retrieve(self, *, workflow_id: str | None = None, k: int = 5) -> list[Lesson]:
        pool = self.lessons
        if workflow_id:
